...
public:
    ...
    // Location info related functions
    ...
    // Notification/Subscription related functions
    ...
    virtual std::shared_ptr<Error> UnregisterNotification(const std::string& email, const std::string& location) = 0;
    virtual Result<std::list<std::string>> GetSubscriptionEmailsByLocation(const std::string& location) = 0;
    virtual Result<std::list<std::string>> GetUpdatedCountryCodeSince(std::chrono::system_clock::time_point time) = 0;
    ...
    // user register related functions
