...
public:
    ...
    std::shared_ptr<Error> UnregisterNotification(const std::string& email, const std::string& location) override;
    Result<std::list<std::string>> GetSubscriptionEmailsByLocation(const std::string& location) override;
    Result<std::list<std::string>> GetUpdatedCountryCodeSince(std::chrono::system_clock::time_point time) override;
